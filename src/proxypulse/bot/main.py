...
async def render_node_detail(node_name: str) -> tuple[str, InlineKeyboardMarkup]:
    ...
    return "\n".join(render_node_lines(node)), build_node_detail_keyboard(node.name)
...
async def node_handler(message: Message, command: CommandObject) -> None:
    ...
    async with SessionLocal() as session:
        node = await get_node_by_name(session, node_name)
    if node is None:
        await message.answer("未找到对应节点。")
        return

    await message.answer("\n".join(render_node_lines(node)), reply_markup=build_node_detail_keyboard(node.name))
